...
Plugin workflows are not resolvable by `name` — always call `Workflow({ scriptPath: "${CLAUDE_PLUGIN_ROOT}/workflows/<name>.js", args })`. If the session did not start inside the repository, pass `repoDir: "<absolute path to the checkout>"` in every args object.
...
2. **Triage** — `Workflow({ scriptPath: "${CLAUDE_PLUGIN_ROOT}/workflows/issue-triage.js", args: { number, repo, cli } })`.
   ...
   **GATE 1** (skip with `--yes`): ask the user to confirm the decision or override it.
   - `close-fixed` / `close-invalid` / `duplicate` / `needs-info`: `Workflow({ scriptPath: "${CLAUDE_PLUGIN_ROOT}/workflows/issue-resolve.js", args: { number, repo, cli, decision, proposedComment, proposedLabels, duplicateOf } })`. Report and stop.
...
3. **Fix** — `Workflow({ scriptPath: "${CLAUDE_PLUGIN_ROOT}/workflows/fix-small.js", args: { number, repo, base: BASE, issue: result.issue, premise: result.evidence.premise, fleetBrief: "${CLAUDE_PLUGIN_ROOT}/instructions/fleet-brief.md" } })`.
   If `ok` is false or `impl.blocked` is set: report why and stop. No PR for an unverified fix.

4. **Review** (only with `--review`) — `Workflow({ scriptPath: "${CLAUDE_PLUGIN_ROOT}/workflows/adversarial-review.js", args: { target: "branch <branch>", diffPath, maxFindings: 6, reviewModel: "sonnet" } })` after producing `diffPath` with `scripts/review-target.sh <branch> <scratchpad>/review.patch`. Critical/high confirmed findings: feed them back through one more `fix-small` run with `branch` set and the findings appended to `issue.claims`; otherwise carry them into the PR checklist.

5. **GATE 2** (skip with `--yes`): show the branch, diff size, tests run, verifier result, and confirmed findings. Ask: open the PR?

6. **PR** — `Workflow({ scriptPath: "${CLAUDE_PLUGIN_ROOT}/workflows/pr-open.js", args: { branch, base: BASE, number, repo, cli, summary: { problem: impl.problem, approach: impl.approach, tests: verify.testsRun }, findings } })` — the summary comes from the fix-small result, not from memory.
