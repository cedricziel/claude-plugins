...
2. **Triage** — `Workflow({ name: "issue-triage", args: { number, repo, cli } })`.
   ...
   **GATE 1** (skip with `--yes`): ask the user to confirm the decision or override it.
   - `close-fixed` / `close-invalid` / `duplicate` / `needs-info`: `Workflow({ name: "issue-resolve", args: { number, repo, cli, decision, proposedComment, proposedLabels, duplicateOf } })`. Report and stop.
