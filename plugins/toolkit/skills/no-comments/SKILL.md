...
<!--
Vendored from https://github.com/cursor/plugins/blob/99559f2f52047978602ef365589275831e76af07/pstack/skills/no-comments/SKILL.md
(pstack by Lauren Tan, MIT — see LICENSE in this directory).
Adapted: `subagent_type` points at the plugin-namespaced agent `toolkit:comment-sicko`
(agents/comment-sicko.md). Not vendored: `/how`, `/why`, `/architect`, and the
`principle-*` skills — where the steps call them, inspect the symbol / sketch the
shape yourself. To resync: fetch the file at the newest commit and update the SHA above.
-->
...
1. Spawn `Task` with `subagent_type: "toolkit:comment-sicko"`. Pass the scope. Do not restate its rules.
