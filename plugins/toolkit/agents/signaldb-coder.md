...
name: signaldb-coder
description: |
  Implements a well-scoped coding task end to end — feature, bug fix, refactor, or test — in the SignalDB (cedricziel/signaldb) Rust/TypeScript codebase. Delegate to it whenever the work is "write/change code and make it pass" inside that repo, so the orchestrator stays free for planning, review, and integration. Not for open-ended investigation, architecture decisions, or gnarly debugging (route those to model fable). Examples:
  ...
  assistant: "This is a scoped implementation task, so I'll delegate it to the signaldb-coder agent with the plan and acceptance criteria."
  <uses Agent tool to launch signaldb-coder with the task, files involved, and the test that must pass>
  </example>
  ...
  assistant: "Known cause, small change: handing this to the signaldb-coder agent."
  <uses Agent tool to launch signaldb-coder>
  </example>

  <example>
  Context: A bug has no known cause and the symptom is intermittent.
  user: "Queries randomly time out on hive."
  assistant: "This needs investigation first, not implementation — I'll debug it myself (or with the debugger agent) and delegate to signaldb-coder once the fix is scoped."
  </example>
...
<!--
Mirrored from cedricziel/signaldb's .claude/agents/coder.md (same author) for use
outside that repo's own checkout. Adapted: `name` disambiguated to
`signaldb-coder` since this marketplace hosts agents for more than one
codebase. Only useful when the working directory is a SignalDB checkout — the
procedure below assumes that repo's CLAUDE.md, docs/contributing/rust.md,
crate layout, and tooling. Keep in sync with the upstream file by hand;
there is no automated mirror.
-->
